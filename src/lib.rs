...
impl<'a> Handler<'a>  {
            ...
            match self.auth_type {
                AuthType::AWS4 => { 
                    res = match self.url_style {
                        UrlStyle::PATH => { std::str::from_utf8(&self.aws_v4_request("GET", None, &format!("/{}", bucket.as_str()), &Vec::new(), Vec::new())?.0).unwrap_or("").to_string() },
                        UrlStyle::HOST => { std::str::from_utf8(&self.aws_v4_request("GET", Some(bucket), "/", &Vec::new(), Vec::new())?.0).unwrap_or("").to_string() }
                    };
                    match self.format {
                        Format::JSON => {
                ...
                AuthType::AWS2 => { 
                    res = std::str::from_utf8(&self.aws_v2_request("GET", &format!("/{}", bucket.as_str()), &Vec::new(), &Vec::new())?.0).unwrap_or("").to_string();
                    match self.format {
                        Format::JSON => {
                            for cap in re.captures_iter(&res) {
                                println!("{}/{}", bucket_prefix, &cap[1]);
    ...
    pub fn ls(&self, bucket:Option<&str>)  -> Result<(), &'static str> {
        ...
        match bucket {
            Some(b) => {
                ...
                match self.auth_type {
                    AuthType::AWS4 => {res = std::str::from_utf8(&self.aws_v4_request("GET", vitural_host.clone(), &uri, &Vec::new(), Vec::new())?.0).unwrap_or("").to_string();},
                    AuthType::AWS2 => {res = std::str::from_utf8(&self.aws_v2_request("GET", &uri, &Vec::new(), &Vec::new())?.0).unwrap_or("").to_string();}
                }
                match self.format {
                    Format::JSON => {
            ...
            None => {
                match self.auth_type {
                    AuthType::AWS4 => {res = std::str::from_utf8(&self.aws_v4_request("GET", None, "/", &Vec::new(), Vec::new())?.0).unwrap_or("").to_string();},
                    AuthType::AWS2 => {res = std::str::from_utf8(&self.aws_v2_request("GET", "/", &Vec::new(), &Vec::new())?.0).unwrap_or("").to_string();}
                }
                match self.format {
                    Format::JSON => {
        ...
        if !Path::new(file).exists() && file == "test"{ 
            ...
            if file_size > 5242880 {
                let res = match self.auth_type {
                    AuthType::AWS4 => {std::str::from_utf8(&self.aws_v4_request("POST", None, &uri, &vec![("uploads", "")], Vec::new())?.0).unwrap_or("").to_string()},
                    AuthType::AWS2 => {std::str::from_utf8(&self.aws_v2_request("POST", &uri, &vec![("uploads", "")], &Vec::new())?.0).unwrap_or("").to_string()}
                };
                ...
                loop {
                    ...
                    let headers = match self.auth_type {
                        AuthType::AWS4 => {self.aws_v4_request("PUT", None, &uri, &vec![("uploadId", upload_id.as_str()), ("partNumber", part.to_string().as_str())], buffer.to_vec())?.1},
                        AuthType::AWS2 => {self.aws_v2_request("PUT", &uri, &vec![("uploadId", upload_id.as_str()), ("partNumber", part.to_string().as_str())], & buffer.to_vec())?.1}
                    };
                ...
                match self.auth_type {
                    AuthType::AWS4 => {self.aws_v4_request("PUT", None, &uri, &Vec::new(), content);},
                    AuthType::AWS2 => {self.aws_v2_request("PUT", &uri, &Vec::new(), &content);}
                };
            };
        }
        Ok(())
    ...
    pub fn get(&self, src:&str, file:Option<&str>) -> Result<(), &'static str> {
        ...
        match self.auth_type {
            AuthType::AWS4 => {
                ...
                match write(fout, self.aws_v4_request("GET", virtural_host, &uri, &Vec::new(), Vec::new())?.0){
                    Ok(_) => return Ok(()),
                    Err(_) => return Err("write file error") //XXX
                }
            },
            AuthType::AWS2 => {
                match write(fout, self.aws_v2_request("GET", &format!("/{}{}", &caps["bucket"], &caps["object"]), &Vec::new(), &Vec::new())?.0){
                    Ok(_) => return Ok(()),
                    Err(_) => return Err("write file error") //XXX
                }
            }
    ...
    pub fn del(&self, src:&str) -> Result<(), &'static str> {
        ...
        match self.auth_type {
            AuthType::AWS4 => {self.aws_v4_request("DELETE", virtural_host, &uri, &Vec::new(), Vec::new());},
            AuthType::AWS2 => {self.aws_v2_request("GET", &format!("/{}{}", &caps["bucket"], &caps["object"]), &Vec::new(), &Vec::new());}
        }
        Ok(())
    }
    ...
    pub fn mb(&self, bucket: &str) -> Result<(), &'static str> {
        ...
        uri.push_str(bucket);
        match self.auth_type {
            AuthType::AWS4 => {self.aws_v4_request("PUT", None, &uri, &Vec::new(), Vec::new());},
            AuthType::AWS2 => {self.aws_v2_request("PUT", &uri, &Vec::new(), &Vec::new());}
        };
        Ok(())
    }
    ...
    pub fn rb(&self, bucket: &str) -> Result<(), &'static str> {
        ...
        uri.push_str(bucket);
        match self.auth_type {
            AuthType::AWS4 => {self.aws_v4_request("DELETE", None, &uri, &Vec::new(), Vec::new());},
            AuthType::AWS2 => {self.aws_v2_request("DELETE", &uri, &Vec::new(), &Vec::new());}
        };
        Ok(())
    }
    ...
    pub fn list_tag(&self, target: &str) -> Result<(), &'static str>  {
        ...
        res = match self.auth_type {
            AuthType::AWS4 => {std::str::from_utf8(&self.aws_v4_request("GET", virtural_host, &uri, &query_string, Vec::new())?.0).unwrap_or("").to_string()},
            AuthType::AWS2 => {std::str::from_utf8(&self.aws_v2_request("GET", &format!("/{}{}", &caps["bucket"], &caps["object"]), &query_string, &Vec::new())?.0).unwrap_or("").to_string()}
        };
	// TODO:
    ...
    pub fn add_tag(&self, target: &str, tags: &Vec<(&str, &str)>) -> Result<(), &'static str>  {
        ...
        let query_string = vec![("tagging", "")];
        match self.auth_type {
            AuthType::AWS4 => {self.aws_v4_request("PUT", virtural_host, &uri, &query_string, content.into_bytes());},
            AuthType::AWS2 => {self.aws_v2_request("PUT", &format!("/{}{}", &caps["bucket"], &caps["object"]), &query_string, &content.into_bytes());}
        }
        Ok(())
    }
